...
program define generate_grids
	...
	* take the mean of each of the tercile (by impact region) of long run tmax
	* generated by emile from the projection system
	* long run tmax from CCSM4 rcp8.5, averaged from 1950 to 2005 
	use "$DB/Global ACP/labor/2_regression/time_use/input/lrtmax_grid.dta", clear
	...
	* income generated by tom
	* also from projection system
	* mean of each of the tercile (by impact region) 

	use "$DB/Global ACP/labor/2_regression/time_use/input/mean_loggdppc_2010_grid.dta", clear
	sum loggdppc, detail
		...
		foreach LL in $LL_list {		
			foreach ref_temp in `center' `default_ref_temp' {	
				...
				* if treatment was not differentiated between high and low risk, gen RF for only low risk
				...
				keep temp ref yhat* se* upper* lower* density significant*
